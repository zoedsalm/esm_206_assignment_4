...
lobster_protection <- lobster_tidy %>% 
  ...
  group_by(year) %>% 
  group_by(site) %>%
  mutate(mean_size = mean(size_mm)) ## Need to select only the means calculated for each site at 2012 + 2018

# Graphing MPA vs. Non-MPA
# I wanted to do a boxplot for each site but boxplots use medians, not means like she asked :(
# I dont like this first one but leaving it for later

ggplot(lobster_protection, aes(x = mean_size, y = site)) +
  geom_density_ridges(aes(fill = site),
                      show.legend = FALSE)+
...
ggplot(lobster_protection, aes(x = site, y = mean_size))+
